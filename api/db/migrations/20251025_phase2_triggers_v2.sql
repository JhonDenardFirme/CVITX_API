-- Phase 2 v2: make defaults apply on UPDATE as well; broaden parent bump; backfill.
...
DO $$
BEGIN
  -- Replace old trigger to run on INSERT *and* UPDATE
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_results_status_default') THEN
    DROP TRIGGER trg_results_status_default ON public.image_analysis_results;
  END IF;
  CREATE TRIGGER trg_results_status_default
    BEFORE INSERT OR UPDATE ON public.image_analysis_results
...
BEGIN
  ...
  -- Uploaded/queued -> processing when any child activity arrives
  ...
  -- When both children are ready and no errors -> done
...
DO $$
BEGIN
  -- Replace old trigger: bump parent on any INSERT or UPDATE (not just when status/error_msg changed)
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_parent_status_bump') THEN
    DROP TRIGGER trg_parent_status_bump ON public.image_analysis_results;
  END IF;
  CREATE TRIGGER trg_parent_status_bump
    AFTER INSERT OR UPDATE ON public.image_analysis_results
    FOR EACH ROW EXECUTE FUNCTION public.image_analysis_parent_status_bump();
END$$;

-- Backfill: set status for any existing rows where it's NULL
UPDATE public.image_analysis_results
   SET status = CASE WHEN COALESCE(error_msg,'') <> '' THEN 'error' ELSE 'ready' END
 WHERE status IS NULL;

-- Update stats so planner has fresh numbers
