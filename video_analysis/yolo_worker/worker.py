...
def _is_valid_bbox(b: Tuple[int,int,int,int], W: int, H: int, min_size: int = 30) -> bool:
    x1,y1,x2,y2 = b
...
def _completeness(b: Tuple[int,int,int,int], W: int, H: int, margin: int = 10) -> float:
    x1,y1,x2,y2 = b
    cuts = int(x1 <= margin) + int(x2 >= (W - margin)) + int(y1 <= margin) + int(y2 >= (H - margin))
    if cuts == 0: return 1.0
    if cuts == 1: return 0.7
    if cuts == 2: return 0.4
    return 0.1

def _quality(b: Tuple[int,int,int,int], W: int, H: int, conf: float) -> Tuple[float, float, float]:
    x1,y1,x2,y2 = b
...
def _send_snapshot_ready(payload: Dict[str, Any]) -> None:
    ...
    log.info(f"[emit] SNAPSHOT_READY track={payload['track_id']} uri={payload['snapshot_s3_key']}")
    ...
    while current_target is not None:
        ...
        for tid, tb in needs_by_frame[current_target]:
            ...
            roi = crop_with_margin(
                ...
                neighbor_iou=max(neighbor_iou, float(CONFIG["SNAPSHOT_NEIGHBOR_IOU"])),
            )
            square = letterbox_to_square(roi, size=int(CONFIG["SNAPSHOT_SIZE"]))
...
def _process_one_video(body: Dict[str, Any]) -> int:
    ...
    if body.get("event") == "PROCESS_VIDEO_DB":
        ...
        full = {
            ...
            "frame_stride": int(vrow.get("frame_stride") or int(CONFIG["FRAME_STRIDE_DEFAULT"])),
    ...
    for tid, tb in track_buf.items():
        if tb.age >= _MIN_TRACK_AGE and tb.hits >= _MIN_HITS and tb.best_box is not None:
            best_by_tid[tid] = tb
...
def _receive_loop() -> None:
    ...
    log.info(f"[boot] yolo_worker ready — queue={qurl} wait={wait}s vis={vis}s :: {CONFIG['AWS_REGION']}")
